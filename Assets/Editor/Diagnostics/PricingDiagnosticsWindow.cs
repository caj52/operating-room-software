...
public sealed class PricingDiagnosticsWindow : EditorWindow
{
    const string DefaultV6Rel =
        "StreamingAssets/Data/quotes/supplier_samples/2026_0501_-_Quote_Request_V6_(Blank).xlsx";
    ...
    static string BuildLog(string supplierPath, PricingBridgeImporter.MatchReport report)
    {
        ...
        sb.AppendLine("Pricing match (same logic as Play Mode Pricing → Load spreadsheet)");
        sb.AppendLine($"Time: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
        sb.AppendLine($"Spreadsheet: {supplierPath}");
        sb.AppendLine($"App price list: {PricingBridgeStore.CatalogPath}");
        sb.AppendLine();
